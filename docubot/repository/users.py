...
async def get_user_profile_by_username(username: str, db: Session) -> RowMapping:
    ...
    user = db.execute(
        select(User.id, User.username, User.first_name, User.last_name,  User.created_at,
               func.count(Document.id).label('number_of_documents'))
        .outerjoin(Document)
...
async def get_users_with_filter(
    ...
    """
    query = db.query(User).options(joinedload(User.documents))
    ...
    if has_documents:
        query = query.filter(exists().where(User.id == Document.user_id))
    else:
        query = query.filter(not_(exists().where(User.id == Document.user_id)))
