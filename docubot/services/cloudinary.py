...
def upload_document(file: BinaryIO, public_id: Optional[str] = None) -> Optional[dict]:
    """
    ...
    """
    try:
        document = cloudinary.uploader.upload_document(
            file=file,
    ...
    return {'url': document.url, 'public_id': document.public_id, 'version': document.version}


def formatting_document_url(public_id: str,
    ...
    document = cloudinary.CloudinaryDocument(
        public_id=public_id,
        version=version,
        url_options=transformation
    )

    return {'url': document.url, 'format': document.url_options}


def remove_document(public_id: str) -> bool:
